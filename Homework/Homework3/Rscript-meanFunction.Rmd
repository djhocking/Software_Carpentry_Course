...
```{r}
meanVector <- function(x) { # We use the function command and give argument (input) x, which will be a vector
  sum.vector <- 0 # Now we set the starting cummulative sum at 0
  for(i in 1:length(x)) { # We will use a for loop which might not be the fastest in R but it's easier to understand when starting off than other options
